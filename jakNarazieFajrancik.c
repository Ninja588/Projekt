...
void drawScore(unsigned int score, unsigned int highscoreInt) {
    int temp = 0;
    int valueArray[10];
    glPushMatrix();
    glLoadIdentity();
    ...
    glRasterPos2f(Settings.resolutionWidth*0.76,Settings.resolutionHeight*0.265);//(1135, 310);
    if (highscoreInt != 0) {
        while(highscoreInt != 0) {
            valueArray[temp] = highscoreInt%10;
            temp++;
            highscoreInt/=10;
        }
        for(int i=temp-1;i>=0;i--) {
...
void renderGridAndTiles() {
    ...
    if(!isBackgroundLoaded) {
        texture = LoadTexture("testGameBack3.bmp");
...
void display2() {
   ...
   // glBindTexture(GL_TEXTURE_2D, texture);
    ...
    texture = LoadTexture("settingsButton.bmp");
    ...
    texture = LoadTexture("startButton.bmp");
...
void display3() {
    ...
    texture = LoadTexture("backButton.bmp");
